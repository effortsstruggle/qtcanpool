...
set(QtCanpoolDocumentation_LIST_DIR ${CMAKE_CURRENT_LIST_DIR})

function(qtc_docs_dir varName)
  if (QtCanpool_SOURCE_DIR)
    # Qt Canpool build or super-repo
    set(${varName} "${QtCanpool_SOURCE_DIR}/doc" PARENT_SCOPE)
  elseif(QtCanpoolDocumentation_LIST_DIR MATCHES /lib/cmake/QtCanpool$)
    # Dev package
    file(RELATIVE_PATH relative_header_path "/${IDE_CMAKE_INSTALL_PATH}/QtCanpool" "/${IDE_HEADER_INSTALL_PATH}")
    set(${varName}
        "${QtCanpoolDocumentation_LIST_DIR}/${relative_header_path}/doc"
        PARENT_SCOPE)
  else()
...
function(qtc_index_dir varName)
  if (QtCanpool_BINARY_DIR)
    # Qt Canpool build or super-repo
    set(${varName} "${QtCanpool_BINARY_DIR}/doc/html" PARENT_SCOPE)
  elseif(QtCanpoolDocumentation_LIST_DIR MATCHES /lib/cmake/QtCanpool$)
    # Dev package
    set(${varName} "${QtCanpoolDocumentation_LIST_DIR}/../../../${IDE_DOC_PATH}" PARENT_SCOPE)
  else()
...
function(add_qtc_documentation qdocconf_file)
  ...
  set(QTC_VERSION "${QTPROJECT_DISPLAY_VERSION}")
  set(QTPROJECT_COPYRIGHT_YEAR "${QTPROJECT_COPYRIGHT_YEAR}")
  string(REPLACE "." "" QTC_VERSION_TAG "${QTPROJECT_VERSION}")
