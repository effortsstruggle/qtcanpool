...
QNavTreeView::QNavTreeView(QWidget *parent)
    : QTreeView(parent)
    ...
    , m_lineColor(QColor("#404244"))
{
    ...
    this->setItemDelegate(new QNavTreeDelegate(this, this));
}

void QNavTreeView::addItem(const QString &text, QStandardItem *parent)
{
    if (parent == nullptr) {
...
void QNavTreeView::addItem(const QString &parentText, const QString &text)
{
    QList<QStandardItem *> ls = m_model->findItems(parentText, Qt::MatchRecursive);

    if (!ls.isEmpty()) {
        foreach(QStandardItem * item, ls) { addItem(text, item); }
    }
}

void QNavTreeView::setItemInfo(const QString &itemText, const QString &info)
{
...
QString QNavTreeView::infoStr(const QModelIndex &index)
{
...
QNavTreeDelegate::QNavTreeDelegate(QNavTreeView *view, QObject *parent)
...
QNavTreeDelegate::~QNavTreeDelegate() {}

QSize QNavTreeDelegate::sizeHint(const QStyleOptionViewItem &option, const QModelIndex &index) const
{
    Q_UNUSED(option);
    Q_UNUSED(index);
    return QSize(50, 30);
...
void QNavTreeDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const
{
    ...
    QBrush decorationBrush(option.state & QStyle::State_Selected ? m_view->checkedTextColor()
                                                                 : m_view->checkedBgColor());
