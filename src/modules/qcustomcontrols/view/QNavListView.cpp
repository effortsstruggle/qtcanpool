...
QNavListView::QNavListView(QWidget *parent)
    : QListView(parent)
    , m_infoVisible(true)
    , m_lineVisible(true)
    ...
    , m_pModel(new QNavListModel(this))
    , m_pDelegate(new QNavListDelegate(this))
{
...
QNavListView::~QNavListView()
{
    delete m_pModel;
    delete m_pDelegate;
}

void QNavListView::readData(const QString &xmlFileName)
{
...
void QNavListView::setData(const QStringList &listItem)
{
...
void QNavListView::setInfoVisible(bool visible) { m_infoVisible = visible; }

void QNavListView::setLineVisible(bool visible) { m_lineVisible = visible; }

void QNavListView::setIconBgcVisible(bool visible) { m_iconBgcVisible = visible; }

void QNavListView::setIconStyle(QNavListView::IconStyle style) { m_style = style; }

void QNavListView::setLineColor(const QColor &color) { m_lineColor = color; }

void QNavListView::setBackColor(const QColor &normalColor, const QColor &selectedColor, const QColor &hoverColor)
{
...
void QNavListView::setTextColor(const QColor &normalColor, const QColor &selectedColor, const QColor &hoverColor)
{
...
QNavListModel::QNavListModel(QObject *parent) : QAbstractListModel(parent) {}

QNavListModel::~QNavListModel()
{
...
void QNavListModel::readData(const QString &xmlFileName)
{
...
void QNavListModel::setItems(const QStringList &listItem)
{
...
void QNavListModel::collapse(const QModelIndex &index)
{
...
int QNavListModel::rowCount(const QModelIndex &parent) const
{
    Q_UNUSED(parent);
    return static_cast<int>(m_listNodes.size());
}

QVariant QNavListModel::data(const QModelIndex &index, int role) const
{
    if (!index.isValid()) {
        return QVariant();
    }
...
void QNavListModel::refreshList()
{
    m_listNodes.clear();
    ...
    for (QList<TreeNode *>::iterator it = m_treeNodes.begin(); it != m_treeNodes.end(); ++it) {
        ...
        for (QList<TreeNode *>::iterator child = (*it)->childrens.begin(); child != (*it)->childrens.end(); ++child) {
            ListNode node;
...
QNavListDelegate::QNavListDelegate(QObject *parent)
    : QStyledItemDelegate(parent), view(qobject_cast<QNavListView *>(parent))
{
}

QNavListDelegate::~QNavListDelegate() {}

QSize QNavListDelegate::sizeHint(const QStyleOptionViewItem &option, const QModelIndex &index) const
{
    Q_UNUSED(option);
    QNavListModel::TreeNode *node = index.data(Qt::UserRole).value<QNavListModel::TreeNode *>();
...
void QNavListDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const
{
...
void QNavListDelegate::drawBranchImage(QPainter *painter, const QStyleOptionViewItem &option,
                                       QNavListModel::TreeNode *node) const
{
    QPixmap pix(18, 16);
    ...
    if (view->iconStyle() == QNavListView::Cross) {
    ...
    } else if (view->iconStyle() == QNavListView::Triangle) {
        p.setPen(Qt::NoPen);
...
void QNavListDelegate::drawTip(QPainter *painter, const QStyleOptionViewItem &option,
                               QNavListModel::TreeNode *node) const
{
    ...
    QPen decorationPen((option.state & QStyle::State_Selected) ? view->selectedBackColor() : view->selectedTextColor());
    QBrush decorationBrush((option.state & QStyle::State_Selected) ? view->selectedTextColor()
                                                                   : view->selectedBackColor());
