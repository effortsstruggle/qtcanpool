...
void QStateProgressBar::setMaximum(int max)
{
    if (max < 1 || max == m_max) {
        return;
    }
...
void QStateProgressBar::setNotes(const QStringList &notes)
{
    if (notes != m_notes) {
        m_notes = notes;
        update();
...
void QStateProgressBar::setStates(const QStringList &states)
{
    if (states != m_states) {
        m_states = states;
        update();
...
void QStateProgressBar::setType(Type type) { m_type = type; }

void QStateProgressBar::setBarColor(const QColor &color)
{
    if (color != m_barColor) {
        m_barColor = color;
        update();
...
void QStateProgressBar::setBarBackColor(const QColor &color)
{
...
void QStateProgressBar::setNoteColor(const QColor &color)
{
...
void QStateProgressBar::setStateColor(const QColor &color)
{
...
void QStateProgressBar::setTipColor(const QColor &color)
{
    if (color != m_tipColor) {
        m_tipColor = color;
        update();
...
void QStateProgressBar::setTipBackColor(const QColor &color)
{
...
QSize QStateProgressBar::sizeHint() const { return QSize(width(), width() / 5); }

QSize QStateProgressBar::minimumSizeHint() const
{
    int w = 125 * m_max;
    return QSize(w, w / 5);
}

void QStateProgressBar::drawBackground(QPainter &p)
{
    p.save();
    p.setPen(Qt::NoPen);
...
void QStateProgressBar::drawBarBackground(QPainter &p)
{
...
void QStateProgressBar::drawBar(QPainter &p)
{
    if (m_value < 1) {
        return;
    }
...
void QStateProgressBar::drawBarNumber(QPainter &p)
{
    p.save();
    p.setPen(Qt::white);
...
void QStateProgressBar::drawNote(QPainter &p, int index, const QColor &color)
{
...
void QStateProgressBar::drawState(QPainter &p, int index, const QColor &color)
{
...
void QStateProgressBar::drawTip(QPainter &p, int index)
{
    if ((m_notes.isEmpty() || index >= m_notes.count() || m_notes[index].isEmpty()) && m_tip.isEmpty()) {
        return;
    }
    QString tip = m_tip;
    if (index < m_notes.count()) {
