...
BackstagePageInfo::BackstagePageInfo(QWidget *parent)
    : QRibbon::RibbonBackstagePage(parent)
{
    ui.setupUi(this);
    createActions();
    createMenus();
...
void BackstagePageInfo::createActions()
{
    ...
    m_actMark->setStatusTip(
        tr("Check the document for content that people with disabilities might find difficult to read."));
}
...
BackstagePageSave::BackstagePageSave(QWidget *parent)
    : QRibbon::RibbonBackstagePage(parent)
{
    m_separator = new QRibbon::RibbonBackstageSeparator(this);
    ...
    if (!str.isEmpty()) {
        ...
        if (index != -1)
            ui.labelVersion->setText(
                str.insert(index + strVersion.length(), QLatin1String(" ") + QLatin1String(QRIBBON_VERSION_STR)));
    }
}
