...
MainWindow::MainWindow(QWidget *parent)
    : RibbonWindow(parent), m_ribbonStyle(qobject_cast<QRibbon::RibbonStyle *>(qApp->style()))
{
    Q_ASSERT(m_ribbonStyle != Q_NULL);

    setWindowTitle(tr("Ribbon Backstage Sample"));
    ...
    setButtonStyleTheme(OfficeStyle::OfficeWhite);
}
...
{
    if (QRibbon::RibbonPage *pageButtons = ribbonBar()->addPage(tr("&Buttons"))) {
        ...
        if (QRibbon::RibbonGroup *groupView = pageButtons->addGroup(tr("View"))) {
    ...
    if (QRibbon::RibbonPage *pageContext = ribbonBar()->addPage(tr("&Context Tab"))) {
