...
public:
    ...
    virtual ~Cursor();
public:
    QPointF posCursor() const
    {
        return m_point;
    }
    void setPosCursor(QPointF p)
    {
        m_point = p;
    }
protected:
...
{
public:
    State(const QString &strNameState, const QString &strNextNameState, StateState, initFunction init,
          workFunction work);
    virtual ~State();
public:
    bool isActive() const
    ...
    QString nameState() const
    {
        return m_strNameState;
    }
    QString nameNextState() const
    {
        return m_strNextNameState;
    }
    ...
    StateState getStateState() const
    {
        return m_stateState;
    }

    void setStatusTip(QString tip)
    {
        m_strStatusTip = tip;
    }
...
public:
    GeomObject *getObject() const
    {
        return m_pEditableObject;
    }
...
public:
    void setAutoCreateObject(bool bAuto)
    {
        m_bAutoCreateObject = bAuto;
    }
    bool isAutoCreateObject() const
    {
        return m_bAutoCreateObject;
    }

    void setHolder(QRibbon::RibbonGroup *pHolder)
    {
        m_holder = pHolder;
    }
public:
...
protected:
    State *addState(QString strNameState, QString strNextNameState, StateState ss, initFunction init,
                    workFunction work);
...
#define EV_INIT(func) (initFunction)(&func)

#define EV_WORK(func) (workFunction)(&func)

#define ADDSTATE(name, nameNext, ss, init, work) addState(name, nameNext, ss, init, work)
...
public:
    ...
    virtual ~LineTool();
public:
    virtual IdTool getIdTool() const
    {
        return it_line;
    }
    virtual bool startTool();
