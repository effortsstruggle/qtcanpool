...
/* State */
State::State(const QString &strNameState, const QString &strNextNameState, StateState ss, initFunction init,
             workFunction work)
{
    m_init = init;
    m_work = work;
...
State *GeometricTool::addState(QString strNameState, QString strNextNameState, StateState ss, initFunction init,
                               workFunction work)
{
    ...
    : GeometricTool(pView, pDoc, pObject)
{
    ...
    pState = ADDSTATE("findRadius", "ready", ss_notDefined, EV_INIT(CircleTool::initCenter),
                      EV_WORK(CircleTool::findRadius));
    ...
    : GeometricTool(pView, pDoc, pObject)
{
    ...
    pState->setStatusTip(tr("Specify the first point of the segment"));
    pState = ADDSTATE("findPoint2", "ready", ss_notDefined, EV_INIT(RectangleTool::initP2),
                      EV_WORK(RectangleTool::findPoint2));
    pState->setStatusTip(tr("Specify the second point of the segment"));
