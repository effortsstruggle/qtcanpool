...
/* ManagerRibbon */
MainWindow::MainWindow() : RibbonWindow(Q_NULL)
{
...
bool MainWindow::eventFilter(QObject *obj, QEvent *event)
{
    ...
    case QEvent::ChildRemoved:
        if (QChildEvent *pEvent = static_cast<QChildEvent *>(event)) {
            if (qobject_cast<QMdiSubWindow *>(pEvent->child()) && m_childCurrent &&
                m_mdiArea->subWindowList().size() == 1) {
    ...
    return QRibbon::RibbonMainWindow::eventFilter(obj, event);
}
...
void MainWindow::createMenuFile()
{
    ...
    if (QAction *actionFile = ribbonBar()->addSystemButton(iconLogo, tr("&File"))) {
        ...
        if (QRibbon::RibbonSystemPopupBar *pPopupBar =
...
{
    if (QRibbon::RibbonPage *pageView = ribbonBar()->addPage(tr("&View"))) {
        if (QRibbon::RibbonGroup *groupDocument = pageView->addGroup(tr("Document Views"))) {
            groupDocument->addAction(QIcon(":/res/normal.png"), tr("&Normal"), Qt::ToolButtonTextUnderIcon);
            groupDocument->addAction(QIcon(":/res/printpreview.png"), tr("&Print Preview"),
                                     Qt::ToolButtonTextUnderIcon);
            m_actionFullScreen = groupDocument->addAction(QIcon(":/res/fullscreen.png"), tr("&Full Screen"),
                                                          Qt::ToolButtonTextUnderIcon);
        ...
        if (QRibbon::RibbonGroup *groupShow_Hide = pageView->addGroup(tr("Show/Hide"))) {
    ...
    if (QRibbon::RibbonPage *pageWindow = ribbonBar()->addPage(tr("&Window"))) {
        if (QRibbon::RibbonGroup *groupWindow = pageWindow->addGroup(tr("Window"))) {
            QAction *actionnewWindow =
                groupWindow->addAction(QIcon(":/res/newwindow.png"), tr("&New Window"), Qt::ToolButtonTextUnderIcon);
...
{
    QRibbon::RibbonMainWindow::keyPressEvent(event);
